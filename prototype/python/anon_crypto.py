...
import M2Crypto.EVP, M2Crypto.RSA, M2Crypto.Rand, M2Crypto.EC
import pyelliptic
...
class AnonCrypto:
	...
	@staticmethod
	def encrypt_with_rsa(pubkey, data):
		return pyelliptic.ECC.encrypt(data, pubkey.get_pubkey())

	@staticmethod
	def decrypt_with_rsa(privkey, msg):
		return privkey.decrypt(msg)
	...
	#def encrypt_with_rsa(pubkey, msg):
		...
		#AnonCrypto.CLEN = len(enc_key)

		#ctext = enc_key + enc_msg
		##print "created " + (":".join("{:02x}".format(ord(c)) for c in enc_msg))
		#return ctext

	#@staticmethod
	#def decrypt_with_rsa(privkey, cipherstr):
		#""" Get the length of the asymmetric part """

		#enc_key = cipherstr[:AnonCrypto.CLEN]	# First 128 bytes are the key
		#enc_msg = cipherstr[AnonCrypto.CLEN:]	# Rest is the padded AES ciphertext
		##print "received" + (":".join("{:02x}".format(ord(c)) for c in enc_msg))
	...
	@staticmethod
	def random_key(bla):
		return pyelliptic.ECC(curve='secp256k1')
    


	@staticmethod
	def random_signing_key():
		keypair = M2Crypto.EC.gen_params(M2Crypto.EC.NID_secp256k1)
		keypair.gen_key()
		return keypair
	...
	def verify(key_dict, msgstr):
		((r_id, msg, (r, s))) = marshal.loads(msgstr)
		if key_dict[r_id][0].verify_dsa(AnonCrypto.hash(msg), r, s):
			return msg
		else:
	...
	@staticmethod
	def priv_key_to_str(k):
		return k.get_privkey()

	@staticmethod
	def priv_key_from_str(s):
		return pyelliptic.ECC(privkey=s,curve='secp256k1')

	@staticmethod
	def pub_key_to_str(k):
		return k.get_pubkey()

	@staticmethod
	def pub_key_from_str(s):
		return pyelliptic.ECC(pubkey=s,curve='secp256k1')
